...
| [Fabric_Admin_Guide](markdown/Fabric_Admin_Guide.md) | [PDF](pdf/Fabric_Admin_Guide.pdf) | **Guide d'administration Microsoft Fabric** (FR) — Panorama complet des rôles (Tenant, Domain, Capacity, Workspace, Item), mise en place d'une administration **par pays** (domaines + capacités + groupes Entra + Multi-Geo), accès programmatique aux **Admin APIs** (service principal, Workload Identity Federation, endpoints, throttling), délégation des **tenant settings** aux Domain/Capacity Admins (liste exhaustive, exclusivités domaine, settings non-délégables), **logs d'utilisation** (M365 Audit, Activity Log, Capacity Metrics, Log Analytics, architecture par pays avec RLS), **restrictions de création d'artefacts** (Lakehouse/Warehouse/Notebook, leviers natifs vs detect & remediate), et **CI/CD avec GitLab** (REST API, fabric-cicd, Workload Identity Federation, pattern miroir Azure DevOps) |
