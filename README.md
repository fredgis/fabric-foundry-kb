...
| [Fabric_OnPrem_Kafka_Connectivity](markdown/Fabric_OnPrem_Kafka_Connectivity.md) | [PDF](pdf/Fabric_OnPrem_Kafka_Connectivity.pdf) | **On-premises Kafka to Microsoft Fabric architecture guide**: five options compared, Eventstream connector VNet injection, Kafka Connect to Eventhouse over HTTPS 443, the Eventstream Kafka endpoint, Azure Event Hubs as an intermediary, and a port 443 relay to Eventstream. Includes network flows, ports, DNS, TLS, HTTP 403 vs TCP 443 diagnosis, a decision tree, and a proof-of-concept plan |
...
| [Fabric_Admin_Guide](markdown/Fabric_Admin_Guide.md) | [PDF](pdf/Fabric_Admin_Guide.pdf) | **Guide d'administration Microsoft Fabric** (FR) — Panorama complet des rôles (Tenant, Domain, Capacity, Workspace, Item), mise en place d'une administration **par pays** (domaines + capacités + groupes Entra + Multi-Geo), accès programmatique aux **Admin APIs** (service principal, Workload Identity Federation, endpoints, throttling), **pattern fondamental de délégation via groupes Entra ID** (convention de nommage, owners métier, PIM, Access Reviews, dynamic groups, Lifecycle Workflows — pour que le Fabric Admin ne soit plus le goulot d'étranglement des accès), délégation des **tenant settings** aux Domain/Capacity Admins (liste exhaustive, exclusivités domaine, settings non-délégables), **logs d'utilisation** (M365 Audit, Activity Log, Capacity Metrics, Log Analytics, architecture par pays avec RLS), **restrictions de création d'artefacts** (Lakehouse/Warehouse/Notebook, leviers natifs vs detect & remediate), et **CI/CD avec GitLab** (REST API, fabric-cicd, Workload Identity Federation, pattern miroir Azure DevOps) |
| [MLinFabric](markdown/MLinFabric.md) | [PDF](pdf/MLinFabric.pdf) | Best practices for ML Model Endpoints in Microsoft Fabric |
| [Fabric_AzureML_Feature_Store](markdown/Fabric_AzureML_Feature_Store.md) | [PDF](pdf/Fabric_AzureML_Feature_Store.pdf) | **Microsoft Fabric × Azure Machine Learning** — feature store integration, architecture, governance and operating model. Two-part guide: Part 1 vulgarises features, feature stores and the Fabric / AML mental model; Part 2 deep-dives into the ADLS Gen2 + OneLake shortcut contract, the **two complementary authoring paths** (Fabric notebook + SDK vs Azure ML Studio / CLI YAML, with decision tree), the end-to-end DS → MLE flow, materialization (offline / online), training with point-in-time joins, online and batch serving (incl. the **Public preview** Fabric Data Factory AML batch activity), interfacing best practices, governance (identity, network, Purview lineage, EU AI Act), the decision matrix, anti-patterns, a production checklist and a public-references annex. Companion drawio: [simple architecture](drawio/Fabric_AzureML_Feature_Store_Simple.drawio) and [detailed reference architecture](drawio/Fabric_AzureML_Feature_Store_Architecture.drawio). |
...
| [Monitoring_Fabric_Data_Agents](markdown/Monitoring_Fabric_Data_Agents.md) | [PDF](pdf/Monitoring_Fabric_Data_Agents.pdf) | **Monitoring Microsoft Fabric Data Agents** — verified August 2026 status of Purview DSPM for AI audit, Foundry/Application Insights tracing, Capacity Metrics (`LlmPlugin` / `AI Query`), SDK and REST API boundaries, unsupported native monitoring surfaces, privacy/PII controls, and a deployable Eventhouse/Power BI observability architecture |
...
| [Foundry_Agent_Development_Guide](markdown/Foundry_Agent_Development_Guide.md) | [PDF](pdf/Foundry_Agent_Development_Guide.pdf) | End-to-end industrialization guide for AI agents on Microsoft Foundry — Microsoft Agent Framework v1.0, Foundry Toolkit for VS Code, agent harness patterns, **Memory** (preview), **Toolbox** (preview, single MCP endpoint), the refreshed **Hosted Agents** (per-session VM-isolated sandbox, scale-to-zero, BYO VNet, Entra Agent ID), reference architecture, CI/CD with `azd`, security/observability/FinOps, production checklist. Companion drawio architecture in [`drawio/Foundry_Agent_Reference_Architecture.drawio`](drawio/Foundry_Agent_Reference_Architecture.drawio) |
| [SAP_Fabric_Connectivity](markdown/SAP_Fabric_Connectivity.md) | [PDF](pdf/SAP_Fabric_Connectivity.pdf) | **SAP to Microsoft Fabric connectivity**: fully audited September 2026 guide covering eight architecture patterns, classic Fabric SAP connectors, the SAP ABAP Add-On Preview, Mirroring through SAP Datasphere, Copy Job CDC status conflicts, semantic federation, Datasphere and OneLake exchange, Eventstream, Open Mirroring partners, SAP BDC Connect status, Direct Lake, the SAP ODP RFC migration risk, and a 59-entry post-June 2026 change ledger |
...
| [Azure_Critical_Multiregion_Web_Architecture](markdown/Azure_Critical_Multiregion_Web_Architecture.md) | [PDF](pdf/Azure_Critical_Multiregion_Web_Architecture.pdf) | **Architecture Azure — Site Web Critique Multi-Régions** (FR) — Guide complet pour architectes : référence à 8 couches (edge, topologie, compute, données, sécurité, observabilité, DevOps, gouvernance), rôle d'APIM Premium et anti-pattern "APIM devant le front", séparation 4-tiers (Front Door + Static Web App + APIM + Backend + Data) avec auth OAuth2 PKCE, évaluation **Azure HorizonDB** (PostgreSQL distribué) vs Cosmos DB / SQL Hyperscale, **failover multi-région non-pairée** (West Europe + Sweden Central) avec lectures multi-régions sur Redis Enterprise Active-Active / Cosmos DB / replicas HorizonDB et impacts du non-pairé (Storage GRS impossible → Object Replication), justification **Front Door Premium** (Private Link aux origines, WAF managé OWASP + Bot Protection + Threat Intelligence) et coût delta vs Standard, **3 stratégies de résilience edge si Front Door tombe** (Dual Edge Azure recommandé via Traffic Manager → Front Door + Application Gateway, multi-CDN avec Cloudflare/Akamai, DNS direct sur AppGw), tests de bascule trimestriels, modèle de coût et anti-patterns. Schémas ASCII détaillés à chaque section. |
...
| [prez/fabric-azureml-feature-store-editorial/](prez/fabric-azureml-feature-store-editorial/) | **Fabric × Azure ML Feature Store** — editorial deck distilling the long-form guide into 30 slides for steering committees. Eight chapters: problem · fundamental insight · target architecture · two authoring paths · materialization/streaming/serving · governance · FinOps · operations & decision. Reuses the simple/detailed architecture PNGs + 7 dedicated Mermaid diagrams. Run with `cd prez/fabric-azureml-feature-store-editorial && npm install && npm run dev` |
