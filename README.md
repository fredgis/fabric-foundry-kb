...
| [Fabric_AzureML_Feature_Store](markdown/Fabric_AzureML_Feature_Store.md) | [PDF](pdf/Fabric_AzureML_Feature_Store.pdf) | **Microsoft Fabric × Azure Machine Learning** — feature store integration, architecture, governance and operating model. Two-part guide: Part 1 vulgarises features, feature stores and the Fabric / AML mental model; Part 2 deep-dives into the ADLS Gen2 + OneLake shortcut contract, the two authoring paths (Fabric notebook + SDK vs Azure ML Studio / CLI YAML), the end-to-end DS → MLE flow, materialization (offline / online), training with point-in-time joins, online and batch serving (incl. the **Public preview** Fabric Data Factory AML batch activity), governance (identity, network, Purview lineage, EU AI Act), the decision matrix, anti-patterns and a production checklist. Companion drawio architecture in [`drawio/Fabric_AzureML_Feature_Store_Architecture.drawio`](drawio/Fabric_AzureML_Feature_Store_Architecture.drawio) |
